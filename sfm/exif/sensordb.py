...
class SensorDB(object):
    ...
    __db_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "./sensordb.txt")
    ...
    def format_result_to_original_case(query, result_in_lower_case):
        ...
        for item in queryList:
            result_dim = list(filter(lambda x: x[0] == item.lower(), result_in_lower_case))
            if len(result_dim) > 0:
    ...
    def get_sersor_dimension(self, query: Union[str, List[str]]) -> Optional[list[Tuple[str, float]]]:
